...
@dataclass
class TaskMessage:
    FORMAT = '<BIIIIIIIIBBBHIIIBBBHHHHIBHH'
    SIZE = struct.calcsize(FORMAT)
    ...
    # halo reuse (forwarding coord -> worker)
    ...
    return_halo: int = 0 # 0: full patch, 1: K top and M bottom rows as halo
    return_halo_top_rows: int = 0
    return_halo_bottom_rows: int = 0
    ...
    def pack(self) -> bytes:
        ...
        data += struct.pack('<BHH', self.return_halo, self.return_halo_top_rows, self.return_halo_bottom_rows)
        return data
